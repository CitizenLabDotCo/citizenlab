...
import { loadProjectEventsSaga } from '../sagas';
import { loadProjectEventsSuccess } from '../actions';

describe('resources/projects/phases sagas', () => {
  describe('loadProjectEvensSaga', () => {
    const mockedAction = {
      payload: mockString,
    };
    const it = sagaHelper(loadProjectEventsSaga(mockedAction));

    it('should have called the correct API', (result) => {
      expect(result).toEqual(call(fetchProjectEvents, mockedAction.payload));
    });
    ...
    it('then, should dispatch loadProjectsSuccess action', (result) => {
      expect(result).toEqual(put(loadProjectEventsSuccess()));
    });
  });
});
