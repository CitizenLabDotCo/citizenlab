...
export function getLocalized(
  ...
  if (!isNilOrError(multiloc) && !isNilOrError(locale) && !isNilOrError(tenantLocales) && tenantLocales.length > 0) {
    ...
    if (isArray(multiloc) && !isEmpty(multiloc) && multiloc[0].content && multiloc[0].locale) {
      ...
      const winnerLocale = candidateLocales.find(locale => multiloc.some(item => item.locale === locale));
      const winner = get(multiloc.find(item => item.locale === winnerLocale), 'content', '');
      return winner;
    }
