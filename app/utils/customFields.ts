...
export function hasCustomFields(customFieldsSchemas, locale) {
  ...
  const customFieldNames = get(customFieldsSchemas, `json_schema_multiloc.${locale}.properties`, null);

  if (!isEmpty(customFieldNames)) {
    forOwn(customFieldNames, (_value, fieldName) => {
      const uiWidget = get(customFieldsSchemas, `ui_schema_multiloc.${locale}.${fieldName}.${'ui:widget'}`, null);
