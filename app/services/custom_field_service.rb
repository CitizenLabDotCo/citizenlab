class CustomFieldService
  ...
  def delete_custom_field_option_values option
    field = option.custom_field
    if field.input_type == 'multiselect'
      # When option is the only selection
      User
        .where("custom_field_values->>'#{field.key}' = ?", [option.key].to_json)
        .update_all("custom_field_values = custom_field_values - '#{field.key}'")
      # When option was selected amongst other values
      User
        .where("(custom_field_values->>'#{field.key}')::jsonb ? :value", value: option.key)
        .update_all("custom_field_values = jsonb_set(custom_field_values, '{#{field.key}}', (custom_field_values->'#{field.key}') - '#{option.key}')")
    else
      # When single select
      User
        .where("custom_field_values->>'#{field.key}' = ?", option.key)
        .update_all("custom_field_values = custom_field_values - '#{field.key}'")
    end
  end
