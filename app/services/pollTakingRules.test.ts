import { pollTakingState } from './pollTakingRules';
...
describe('pollTakingState', () => {
  describe('continuous projects', () => {
    describe('no restrictions', () => {
      it('allows unsigned users', () => {
        expect(pollTakingState({ project: pollContinuous, signedIn: false }))
          .toEqual({
            enabled: true,
      ...
      it('allows signed users', () => {
        expect(pollTakingState({ project: pollContinuous, signedIn: true }))
          .toEqual({
            disabledReason: undefined,
            enabled: true,
          });
    ...
    describe('not_permitted', () => {
      ...
      it('disabled with maybeNotPermitted for signed out users', () => {
        const accessDeniedProject = getDisabledPollProject('not_permitted');
        expect(pollTakingState({ project: accessDeniedProject, signedIn: false }))
          .toEqual({
            enabled: false,
    ...
    describe('project_inactive', () => {
      ...
      // but pollTakingState doesn't check much further than the action_descriptor.
    ...
    describe('not_poll', () => {
      ...
      it('is not enabled for unsigned users', () => {
        const accessDeniedProject = getDisabledPollProject('not_poll');
        expect(pollTakingState({ project: accessDeniedProject, signedIn: false }).enabled).toBe(false);
      });
