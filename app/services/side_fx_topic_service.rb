class SideFxTopicService
  ...
  def after_destroy frozen_topic, user
    ...
    LogActivityJob.perform_later(encode_frozen_resource(frozen_topic), 'deleted', user, Time.now.to_i, payload: {topic: serialized_topic})
  end
