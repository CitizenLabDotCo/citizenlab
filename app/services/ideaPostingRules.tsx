...
export const getPostingPermission = ({ project, phase, authUser }: PostingButtonStateArg) => {
  ...
  if (!isNilOrError(project) && !isNilOrError(phase)) {
    ...
    const { disabled_reason, future_enabled } = project.attributes.action_descriptor.data.posting;

    if (inCurrentPhase) {
      return {
        show: phase.attributes.participation_method === 'ideation' && phase.attributes.posting_enabled && disabled_reason !== 'not_ideation',
        enabled: (loggedInAsAdmin || project.attributes.action_descriptor.data.posting.enabled),
    ...
    const enabled = (loggedInAsAdmin || project.attributes.action_descriptor.data.posting.enabled);
    const { disabled_reason, future_enabled } = project.attributes.action_descriptor.data.posting;
