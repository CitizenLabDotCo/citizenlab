class ProjectHolderService
  ...
  def fix_project_holder_orderings!
    ...
    ProjectHolderOrdering.create(folder_ids.map{|id| {project_holder_id: id, project_holder_type: 'Folder'}})
    ProjectHolderOrdering.where(project_holder_type: 'Folder').where.not(project_holder_id: folder_ids).destroy_all
