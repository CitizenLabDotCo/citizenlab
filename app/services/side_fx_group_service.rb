class SideFxGroupService
  ...
  def after_destroy frozen_group, user
    ...
    LogActivityJob.perform_later(encode_frozen_resource(frozen_group), 'deleted', user, Time.now.to_i, payload: {group: serialized_group})
  end
