...
export const getNotification = (
  id: string,
  type: 'project_phase_upcoming' | 'project_phase_started' | 'mention_in_official_feedback' | 'official_feedback_on_commented_idea' | 'official_feedback_on_voted_idea' | 'official_feedback_on_your_idea' | 'new_idea_for_admin' | 'admin_rights_received' | 'project_moderation_rights_received' | 'new_comment_for_admin' | 'comment_on_your_comment' | 'comment_on_your_idea' | 'mention_in_comment' | 'idea_marked_as_spam' | 'comment_marked_as_spam' | 'status_change_of_your_idea' | 'invite_accepted' | 'comment_deleted_by_admin',
  ...
  attributes: {
    ...
    initiating_user_first_name: ['status_change_of_your_idea', 'admin_rights_received'].indexOf(type) === -1 ? 'Initiating' : undefined,
    initiating_user_last_name: ['status_change_of_your_idea', 'admin_rights_received'].indexOf(type) === -1 ? 'User' : undefined,
    initiating_user_slug: ['status_change_of_your_idea', 'admin_rights_received'].indexOf(type) === -1 ? 'initiating-user' : undefined,
    idea_title: {
      'nl-BE': 'Qui nihil veniam consequatur.'
    },
    reason_code: ['comment_deleted_by_admin'].indexOf(type) === -1 ? undefined : 'irrelevant',
  },
  relationships: {
    // recipient: {
    //   data: {
    ...
    invite: ['invite_accepted'].indexOf(type) !== -1 ? {
      data: {
        id: 'iviteId',
        type: 'invite'
      }
