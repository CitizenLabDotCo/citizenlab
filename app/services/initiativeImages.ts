...
export function initiativeImagesStream(initiativeId: string, streamParams: IStreamParams | null = null) {
...
export function deleteInitiativeImage(initiativeId: string, imageId: string) {
  return streams.delete(`${API_PATH}/initiatives/${initiativeId}/images/${imageId}`, imageId);
}

export async function addInitiativeImage(initiativeId: string, base64: string, ordering: number | null = null) {
  const apiEndpoint = `${API_PATH}/initiatives/${initiativeId}/images`;
  const bodyData = {
    image: {
      ordering,
  ...
  const initiativeImage = await streams.add<IInitiativeImage>(apiEndpoint, bodyData);
