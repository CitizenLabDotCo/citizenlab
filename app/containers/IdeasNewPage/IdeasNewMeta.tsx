...
interface Props extends InputProps, DataProps, InjectedIntlProps, InjectedLocalized { }

const IdeasNewMeta = React.memo<Props>(({ intl, authUser, locales, project, localize }) => {
  const { formatMessage } = intl;
  const { location } = window;
  const projectName = !isNilOrError(project) && localize(project.attributes.title_multiloc);
  const ideasIndexTitle = formatMessage(messages.metaTitle, { projectName });
  const ideasIndexDescription = formatMessage(messages.metaDescription, { projectName });
