...
export const Indicator = styled(TextCell) <({ isWarning?: boolean }) >`
  display: flex;
  color: ${({ isWarning }) => isWarning ? colors.adminOrangeIcons : colors.clRed};
`;
...
const WrongOptionsIndicator = ({ options }: Props) => !isNilOrError(options) ? options.length === 0 ? (
  <Indicator>
  ...
  <Indicator>
    <StyledIconTooltip content={<FormattedMessage {...messages.oneOptionsTooltip} />} />
