...
const InitiativeMeta = memo<Props & InjectedIntlProps & InjectedLocalized>(({
  ...
  if (!isNilOrError(locale) && !isNilOrError(tenant) && !isNilOrError(initiative)) {
    const { title_multiloc, body_multiloc } = initiative.attributes;
    const tenantLocales = tenant.attributes.settings.core.locales;
    const localizedTitle = localize(title_multiloc, 50);
    const initiativeTitle = formatMessage(messages.metaTitle, { initiativeTitle: localizedTitle });
    const initiativeDescription = stripHtml(localize(body_multiloc), 250);
    const initiativeImage = !isNilOrError(initiativeImages) && initiativeImages.length > 0 ? initiativeImages[0].attributes.versions.fb : null;
    const initiativeUrl = window.location.href;
    const initiativeOgTitle = formatMessage(messages.metaOgTitle, { initiativeTitle: localizedTitle });
    const initiativeAuthorName = !isNilOrError(author) ? `${author.attributes.first_name} ${author.attributes.last_name}` : 'anonymous';
    ...
    return (
      <Helmet>
        ...
        <meta property="og:description" content={formatMessage(messages.initiativeOgDescription)} />
        {initiativeImage && <meta property="og:image" content={initiativeImage} />}
        <meta property="og:image:width" content={`${imageSizes.initiativeImg.fb[0]}`} />
        <meta property="og:image:height" content={`${imageSizes.initiativeImg.fb[1]}`} />

        <meta name="twitter:title" content={initiativeOgTitle} />
        <meta name="twitter:description" content={formatMessage(messages.initiativeOgDescription)} />
        {initiativeImage && <meta name="twitter:image" content={initiativeImage} />}
...
const Data = adopt<DataProps, InputProps>({
  ...
  author: ({ initiative, render }) => <GetUser id={get(initiative, 'relationships.author.data.id', null)}>{render}</GetUser>,
  locale: <GetLocale />,
  tenant: <GetTenant />,
  authUser: <GetAuthUser />,
});

const InitiativeMetaWithHoc = injectIntl<Props>(injectLocalize<Props & InjectedIntlProps>(InitiativeMeta));
