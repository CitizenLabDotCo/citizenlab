...
// services
import { canModerateInitiative } from 'services/permissions/rules/initiativePermissions';
...
class InitiativeComment extends PureComponent<Props & InjectedIntlProps, State> {
  static defaultProps = {
  ...
  render() {
    const { comment, author, initiativeId, commentType, hasBottomBorder, hasChildComments, last, className, canReply } = this.props;
    ...
    if (!isNilOrError(comment)) {
      ...
      const moderator = !isNilOrError(author) && canModerateInitiative({ data: author });
...
const InitiativeCommentWithHoCs = injectIntl<Props>(InitiativeComment);

export default (inputProps: InputProps) => (
  <Data {...inputProps}>
    {dataProps => <InitiativeCommentWithHoCs {...inputProps} {...dataProps} />}
  </Data>
);
