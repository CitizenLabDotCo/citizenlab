...
class CLMap extends React.PureComponent<Props & InjectedLocalized, State> {
  ...
  private map: Leaflet.Map = undefined as any;
  ...
  */
  createLeafletLayers = (layers: IMapLayerAttributes[]) => {
    // first reset layers
    if (this.map) {
  ...
  initMap = () => {
    ...
    if (
      ...
      const leafletLayers = this.createLeafletLayers(layers);
      ...
      this.layerControl = Leaflet.control
        .layers(undefined, overlayMaps)
        .addTo(this.map);
    }
  };
