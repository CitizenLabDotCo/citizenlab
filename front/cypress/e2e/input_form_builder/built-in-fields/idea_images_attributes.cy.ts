...
describe('Input form builder', () => {
  ...
  it('allows user to turn on / off the images default field but not edit its question title', () => {
    ...
    // Check that the images field is present on the idea form before turning it off
    ...
    // Go to the next page of the idea form that has the images field
    cy.dataCy('e2e-next-page').should('be.visible').click();
    cy.wait(1000);
    ...
    // The images tool box item should be disabled as it is already on the canvas
    cy.dataCy('e2e-idea-images-attributes-item').as('imagesToolboxItem');
    cy.get('@imagesToolboxItem').should('exist');
    cy.get('@imagesToolboxItem').should('have.attr', 'disabled');
    ...
    // Find the row containing "Images" and click its "more actions" button
    cy.dataCy('e2e-form-fields')
      .find('[data-cy="e2e-field-row"]')
      .filter(':contains("Question"):contains("Images")')
      .find('[data-cy="e2e-more-field-actions"]')
      .click();
    cy.get('.e2e-more-actions-list button').contains('Delete').click();

    // The Input images tool box item should be enabled as it has been removed from the canvas
    cy.get('@imagesToolboxItem').should('not.have.attr', 'disabled');

    // Check to see that images field is removed from the canvas
    cy.dataCy('e2e-form-fields').within(() => {
      cy.contains('Input images').should('not.exist');
    });
    ...
    // Go to the page that had the images field
    cy.dataCy('e2e-next-page').should('be.visible').click();
    cy.wait(1000);

    cy.get('[data-cy="e2e-idea-image-upload"]').should('not.exist');
  });
});
