...
describe('Input manager', () => {
  ...
  describe('Assignee filter', () => {
    it('Filters on All ideas', () => {
      ...
      cy.get('#e2e-select-assignee-filter').should('be.visible');
      // select should have option with value 'Any administrator'
      cy.get('#e2e-select-assignee-filter').contains('Any administrator');
      ...
      cy.getAuthUser().then((user) => {
        ...
        cy.apiCreateProject({
          ...
          .then((phase) => {
            ...
            cy.get('#e2e-select-assignee-filter').select('Assigned to me');
