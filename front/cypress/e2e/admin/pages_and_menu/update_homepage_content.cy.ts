...
describe('Admin: update HomePage content', () => {
  ...
  it('updates top and bottom info section content and visibility', () => {
    ...
    cy.get('[data-testid="admin-edit-button-top_info_section_enabled"]').click();
    ...
    cy.get('[data-testid="admin-edit-button-bottom_info_section_enabled"]').click();
    cy.get('#custom-section-en').clear().type(bottomInfoContent);
    cy.get('[data-testid="bottom-info-section-submit"]').click();

    cy.visit('/')
    
    cy.get('[data-testid="e2e-landing-page-top-info-section"]').should('not.exist');
    cy.get('[data-testid="e2e-landing-page-bottom-info-section"]').should('not.exist');
    ...
    cy.visit('/')
    cy.get('[data-testid="e2e-landing-page-top-info-section"]').contains(topInfoContent);
    cy.get('[data-testid="e2e-landing-page-bottom-info-section"]').contains(bottomInfoContent);
  });
});
