...
describe('Content builder Container component', () => {
  ...
  it('handles Container component correctly', () => {
    ...
    cy.get('#e2e-single-column').should('have.length', 1); // Only original root container
    ...
    cy.get('#e2e-text-box').should('have.length', 2);
    cy.get('#e2e-about-box').should('have.length', 2);
    cy.get('#e2e-iframe').should('have.length', 2);
    cy.get('#e2e-image').should('have.length', 2);
