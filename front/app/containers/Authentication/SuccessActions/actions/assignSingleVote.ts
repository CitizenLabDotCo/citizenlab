...
export interface AssignSingleVoteParams {
  ideaId: string;
...
export const assignSingleVote =
  ({
    ideaId,
    participationContextId,
    participationContextType,
    basket,
  }: AssignSingleVoteParams) =>
  async () => {
    ...
    if (!isNil(basket)) {
      ...
      try {
        ...
        queryClient.invalidateQueries({
          queryKey: basketsKeys.item({ id: basket.data.id }),
        });
        queryClient.invalidateQueries({
          queryKey: basketsIdeasKeys.list({ basketId: basket.data.id }),
        });
