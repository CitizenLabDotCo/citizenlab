import { defineMessages } from 'react-intl';

export default defineMessages({
  topInfoPageTitle: {
    id: 'app.containers.Admin.pagesAndMenu.topInfoPageTitle',
    defaultMessage: 'Top Info Section',
  },
  topInfoContentEditorTitle: {
    id: 'app.containers.Admin.pagesAndMenu.topInfoContentEditorTitle',
    defaultMessage: 'Content',
  },
  topInfoDescription: {
    id: 'app.containers.Admin.pagesAndMenu.topInfoDescription',
    // get real copy
    defaultMessage:
      'The area at the top of the homepage can be customized with your own images, text and links.',
  },
  topInfoSaveButton: {
    id: 'app.containers.Admin.pagesAndMenu.topInfoSaveButton',
    defaultMessage: 'Save top info section',
  },
});
