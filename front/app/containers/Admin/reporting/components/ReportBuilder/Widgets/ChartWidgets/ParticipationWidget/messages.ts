import { defineMessages } from 'react-intl';

export default defineMessages({
  participation: {
    id: 'app.containers.Admin.reporting.components.ReportBuilder.Widgets.ChartWidgets.ParticipationWidget.participation',
    defaultMessage: 'Participation',
  },
  inputs: {
    id: 'app.containers.Admin.reporting.components.ReportBuilder.Widgets.ChartWidgets.ParticipationWidget.inputs',
    defaultMessage: 'Inputs',
  },
  comments: {
    id: 'app.containers.Admin.reporting.components.ReportBuilder.Widgets.ChartWidgets.ParticipationWidget.comments',
    defaultMessage: 'Comments',
  },
  votes: {
    id: 'app.containers.Admin.reporting.components.ReportBuilder.Widgets.ChartWidgets.ParticipationWidget.votes',
    defaultMessage: 'Votes',
  },
  showInputs: {
    id: 'app.containers.Admin.reporting.components.ReportBuilder.Widgets.ChartWidgets.ParticipationWidget.showInputs',
    defaultMessage: 'Show inputs',
  },
  showComments: {
    id: 'app.containers.Admin.reporting.components.ReportBuilder.Widgets.ChartWidgets.ParticipationWidget.showComments',
    defaultMessage: 'Show comments',
  },
  showVotes: {
    id: 'app.containers.Admin.reporting.components.ReportBuilder.Widgets.ChartWidgets.ParticipationWidget.showVotes',
    defaultMessage: 'Show votes',
  },
  dateColumn: {
    id: 'app.containers.Admin.reporting.components.ReportBuilder.Widgets.ChartWidgets.ParticipationWidget.dateColumn',
    defaultMessage: 'Date',
  },
  participantsCaption: {
    id: 'app.containers.Admin.reporting.components.ReportBuilder.Widgets.ChartWidgets.ParticipationWidget.participantsCaption',
    defaultMessage:
      'Participation data table showing date, participation types, and their values',
  },
});
