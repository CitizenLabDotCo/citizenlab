import { defineMessages } from 'react-intl';

export default defineMessages({
  report: {
    id: 'app.containers.Admin.projects.project.information.ReportTab.report',
    defaultMessage: 'Report',
  },
  createAReportTo: {
    id: 'app.containers.Admin.projects.project.information.ReportTab.createAReportTo',
    defaultMessage: 'Create a report to:',
  },
  shareResults: {
    id: 'app.containers.Admin.projects.project.information.ReportTab.shareResults',
    defaultMessage: 'Share results of a past survey or ideation phase',
  },
  createAMoreComplex: {
    id: 'app.containers.Admin.projects.project.information.ReportTab.createAMoreComplex',
    defaultMessage: 'Create a more complex page for information sharing',
  },
  thisWillBe: {
    id: 'app.containers.Admin.projects.project.information.ReportTab.thisWillBe',
    defaultMessage:
      'This will be publicly available to users as soon as this phase starts.',
  },
  createReport: {
    id: 'app.containers.Admin.projects.project.information.ReportTab.createReport',
    defaultMessage: 'Create a report',
  },
  areYouSureYouWantToDelete: {
    id: 'app.containers.Admin.projects.project.information.areYouSureYouWantToDelete',
    defaultMessage:
      'Are you sure you want to delete this report? This action cannot be undone.',
  },
});
