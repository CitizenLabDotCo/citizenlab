...
import messages from '../../../projects/project/phase/messages';
import PermissionTooltipMessage from '../../../projects/project/phase/PermissionTooltipMessage';
import { getParticipantMessage } from '../../../projects/project/phase/utils';
import { IPhaseTab } from '../../../projects/project/tabs';

const Container = styled(Box)`
  ${defaultCardStyle};
`;

const StandaloneTab = styled.div<{ active: boolean; disabled?: boolean }>`
  list-style: none;
  display: flex;
  align-items: center;
  margin-bottom: -2px;
  ...
  &:hover {
    ...
    ${({ active }) => !active && `border-color: #ddd;`}
  }
...
export const StandalonePhaseHeader = ({
  phase,
  tabs,
  activeTab,
  onTabChange,
  ...
  const handleTabClick = (tab: IPhaseTab) => {
    if (!tab.disabledTooltipText) {
        ...
        >
          {tabs.map((tab) => (
            <Tooltip
              ...
              placement="bottom"
              theme={''}
              disabled={!tab.disabledTooltipText}
              maxWidth={350}
              content={tab.disabledTooltipText}
            >
              <StandaloneTab
