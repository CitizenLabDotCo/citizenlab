...
/**
 * Creates a key-value table (2 columns: label, value).
 */
export function createKeyValueTable(
  data: { label: string; value: string | number }[],
  options: {
    labelWidth?: number; // Percentage
    headerRow?: { label: string; value: string };
  } = {}
): Table {
  const { labelWidth = 40, headerRow } = options;

  const rows: TableRowData[] = [];

  if (headerRow) {
    rows.push({
      cells: [headerRow.label, headerRow.value],
      isHeader: true,
    });
  }
  ...
  data.forEach((item) => {
    rows.push({
      cells: [item.label, item.value.toString()],
      isHeader: false,
    });
  });

  return createTable(rows, {
    columnWidths: [labelWidth, 100 - labelWidth],
    headerRow: !!headerRow,
  });
}
 ...
 * Useful for building tables incrementally.
 */
export function createRowFromCells(
  cells: CellContent[],
  options: {
    isHeader?: boolean;
    columnWidths?: number[];
  ...
  const { isHeader = false, columnWidths, borderColor } = options;

  const backgroundColor = isHeader
    ? WORD_TABLE_STYLES.headerBackground
    : WORD_TABLE_STYLES.rowBackground;
