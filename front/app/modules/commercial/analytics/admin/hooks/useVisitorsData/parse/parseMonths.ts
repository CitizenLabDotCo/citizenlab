...
export const getEmptyMonthsBefore = (startAtMoment: Moment, firstDateInData: Moment) => {
  const months = interpolateMonths(startAtMoment, firstDateInData.subtract({ month: 1 }))
...
export const getEmptyMonthsAfter = (endAtMoment: Moment, lastDateInData: Moment) => {
  const months = interpolateMonths(lastDateInData.add({ month: 1 }), endAtMoment)
  if (months === null) return [];
...
const interpolateMonths = (from: Moment, to: Moment) => {
  ...
  if (fromYear === toYear) return monthRange(fromMonth, toMonth, fromYear);
...
const monthRange = (from: number, to: number, year: number) => {
  return range(from, to).map((month) => moment(getFirstDayOfMonth(year, month)))
}
