...
describe('customPageAttributeDrafts', () => {
  ...
  it('strips the draft before the layout is stored, keeping the other props', () => {
    const stripped = stripCustomPageAttributeDrafts(
      nodesWithTitle({ title: { en: 'About us' }, showTitle: false })
    );

    expect(stripped.T.props).toEqual({ showTitle: false });
  });
