...
- **Ideation → voting.** A process is flat (one config for its life), so ideation→voting on the same ideas is **two processes sharing a form**: a "collect ideas" process owns the ideas and their form; a "vote" process **references that shared form** (the data-set anchor) and operates on the inputs linked to it — the `ideas_phases` join, generalised. Nothing is copied, and no form-compatibility check arises; because the anchor is the form (not one activity's ideas), an admin could also post options, or a later ideation feed the same set. _The Grid does this as **one** process whose config changes per phase (ideas + votes in one data set); Free-floating's flat processes lean on the shared form + join instead._
...
Seen as a **path from Method components**, there are three steps: ① **method → Process** (a real data-set owner); ② **detach time from the timeline** (own dates; the timeline narrates); ③ **give the process an internal lifecycle** (dated steps). The **Grid** takes ① only (keeps the timeline gating); **Free-floating** takes ①+② (a Process on free dates, but still _flat_ — which is why it still leans on a shared form to chain ideation→voting); **Self-contained** takes ①+②+③. So Free-floating is the **midpoint** between Method components and Self-contained, and the shared-form link it relies on is the visible mark of having skipped ③.
...
**A note on "by reference."** The link between an input and the phases / steps / activities it belongs to is a many-to-many in _every_ candidate — it is today's `ideas_phases` with the right-hand side swapped (phases in the Grid, steps in Self-contained, activities in the others). So "voting by reference" really means "voting reads the inputs linked to it, and references the shared **form**" — not a special mechanism. What genuinely differs is **one reconfigured unit** (Grid / Self-contained: voting is a config or step of the _one_ process) vs **two units sharing the form** (Multiple timelines / Method components / Free-floating: ideation and voting are separate units linked through the shared form).
...
**Two axes hide inside this ladder**, and they are unlocked by different moves — worth separating, because it sharpens what the Process actually buys:

- A **configurable, first-class _form_** (one shared form with per-field visibility) unlocks the **visibility** prizes: mixing public + private in one submission, per-submission publish, and multiple / evolving form shapes. These need a real _form_, **not** a real Process — a shared-form-and-activities model reaches them too.
- A **first-class _data set_ (the Process)** unlocks the **lifecycle** prizes: a record that **matures across stages** (collect a rough idea → enrich it with budget / feasibility → vote, on _one_ record), a **status workflow** gating interactions, and a single **input manager** over the whole set.

So the honest split: the form-first-class step buys _visibility_; the data-set-first-class step (the Process) buys _lifecycle_. It is why Method components and Multiple timelines sit low — they lack even a configurable shared form — and why the Process specifically earns its keep on **data-set lifecycle**, not visibility.
...
- **Self-contained processes** — the **stretch destination**: the highest ceiling (a fleet of independently-scheduled, multi-step processes with roll-up — PB across neighbourhoods), but reached _via_ the Grid (on the phase-anchored path) or _via_ Free-floating (on the free-dated path) — not by a direct migration that would invert the timeline.
...
**The free-date choice picks the branch — and it is made early.** "Reach Self-contained _via the Grid_" holds only on the **phase-anchored** path: keep participation snapped to phases, reach the Grid, then let a process's config detach into its own steps. The moment an _extra_ is granted a **genuinely free date** (the June direction), near-term work has stepped onto the **free-floating branch** — and from there Self-contained is reached **via Free-floating, not the Grid** (re-anchoring free-dated data onto a phase grid is the painful move [the time model](#the-time-model-the-startend-mid-phase-question) warns about). So the free-floating-dates fork is not merely the one deliberate decision; it is effectively **decided the first time a free date is granted**, and it determines which destination stays cleanly reachable.
