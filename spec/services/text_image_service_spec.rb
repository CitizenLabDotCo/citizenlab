...
describe TextImageService do
  ...
  describe "swap_data_images_text" do
    let(:url_from_base64) { "/from/base64/image.jpg" }
    let(:url_from_url) { "/from/url/image.jpg" }
    ...
    it "retains all other attributes on the img tag" do
      ...
      HTML
      output = <<~HTML
        <img src="#{url_from_base64}" data-something="1" class="right" target="_blank">
      HTML
      expect(service.swap_data_images_text(input, &image_url_builder)).to eq output
    end
