  ...
  post "web_api/v1/taggings/generate" do
    ...
    example "Generates taggings from new tags" do
      response = [
        ...
        {
          "predicted_labels" => [{"confidence" => 0.599170446395874, "id" => 0}, {"confidence" => 0.599170446395874, "id" => 1}],
          "id" => @ideas[1].id
        },
        {
          "predicted_labels" => [{"confidence" => 0.599170446395874, "id" => 3}, {"confidence" => 0.599170446395874, "id" => 1}],
          "id" => @ideas[3].id
        },
        {
          "predicted_labels" => [{"confidence" => 0.599170446395874, "id" => 2}, {"confidence" => 0.599170446395874, "id" => 4}, {"confidence" => 0.599170446395874, "id" => 1}],
          "id" => @ideas[2].id
        }
      ]
      allow_any_instance_of(NLP::TaggingSuggestionService).to receive(:suggest).and_return(response)
      ...
      expect(response_status).to eq 200
      json_response = json_parse(response_body)
      expect(json_response[:data].map { |el| Tagging::Tag.find(el[:attributes][:tag_id])[:title_multiloc]["en"] }).to match_array ["Lalalal", "Lalalal", "chachacha", "chachacha", "chachacha", "leela", "lilila", "lou"]
      tag_ids = json_response[:data].map { |el| el[:attributes][:tag_id] }
      expect(tag_ids.map { |el| tag_ids.count(el)}).to match_array [2, 2, 3, 3, 3, 1, 1, 1]
    end
