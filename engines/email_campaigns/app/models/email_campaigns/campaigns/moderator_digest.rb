module EmailCampaigns
  class Campaigns::ModeratorDigest < Campaign
    ...
    def statistics project
      ...
      participants_increase = ps.project_participants(project, since: (Time.now - days_ago)).size
      participants_past_increase = ps.project_participants(project, since: (Time.now - (days_ago * 2))).size - participants_increase
      ...
      votes = Vote.where(votable_id: (ideas.map(&:id) + comments.map(&:id)))
      {
        ...
        users: {
          ...
          total_participants: ps.project_participants(project).count
        }
      }
    end
