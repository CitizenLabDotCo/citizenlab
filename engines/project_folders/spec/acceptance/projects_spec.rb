  ...
  context 'when project folder moderator' do
    ...
    let(:other_projects) do
      Project.includes(:admin_publication)
             .where(admin_publications: { publication_status: publication_statuses, parent_id: nil })
    end

    let(:moderated_projects) do
      Project.includes(:admin_publication)
             .where(
               admin_publications: {
             ...
             ).or(Project.includes(:admin_publication).where(projects: { id: user.moderatable_project_ids }))
    end
    ...
    before do
      ...
      user.add_role('project_moderator', { project_id: other_projects.first.id })
      user.save
    end
    ...
    get 'web_api/v1/projects' do
      ...
      example_request 'Lists projects that belong to a folder the user moderates' do
        ...
        expect(ids.length).to eq moderated_projects.length
        expect(ids).to match_array moderated_projects.pluck(:id)
      end
    end
    ...
    post "web_api/v1/projects" do
      ...
      describe do
        ...
        example "Create a project within a folder the user moderates" do
