    ...
    Tenant.all.each do |tenant|
      Apartment::Tenant.switch(tenant.schema_name) do
        ...
        deleted_area_ids.each do |area_id|
          service.delete_custom_field_option_values area_id, CustomField.find_by(key: 'domicile')
        end
      end
    end
  end
    ...
    Tenant.all.each do |tenant|
      Apartment::Tenant.switch(tenant.schema_name) do
        ...
        affected_users.each do |user|
          user.update_attributes custom_field_values: service.cleanup_custom_field_values!(user.custom_field_values)
        end
      end
    end
  end
