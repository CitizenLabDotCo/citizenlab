...
def object_from_template classname, attributes
  ...
  when 'Event'
    nil
  when 'CustomField'
    CustomField.for_resource_type('User').where(code: attributes['code']).first
  when 'CustomFieldOption'
    CustomFieldOption.where(key: attributes['key'], custom_field: CustomField.for_resource_type('User').where(code: attributes['custom_field_ref']['code'])).first
  else
    nil
  end
end
