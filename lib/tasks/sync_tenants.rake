...
namespace :sync_tenants do
  ...
  task :output_updates => [:environment] do |t, args|
      ...
      Apartment::Tenant.switch(tenant.schema_name) do
        template['models'].each do |model_name, fields|
          ...
          fields.each do |attributes|
            ...
            if object.present? && (object.created_at == object.updated_at)
              attributes.each do |field_name, field_value|
                ...
                if value && value_changed(value, object.send(field_name), field_name)
                  output[tenant.host] ||= {}
                  output[tenant.host][classname] ||= {}
                  output[tenant.host][classname][object.id] ||= {}
                  output[tenant.host][classname][object.id][field_name] = value
                end
              end
            end
          end
    ...
    #   end
    # end
    File.open("tmp/output_updates.json", 'w') { |f| f.write output.to_json }
  end
...
def value_changed value1, value2, field_name
  if field_name =~ /_multiloc$/
    ...
    (value1.keys & value2.keys).any? do |key|
