    ...
    stage('Deploy to staging') {
      steps {
        sshagent (credentials: ['local-ssh-user']) {
          ...
          sh 'ssh -t -o StrictHostKeyChecking=no -l ubuntu 35.157.143.6 cd cl2-deployment && sudo docker stack deploy --compose-file docker-compose-staging.yml cl2-back-stg --with-registry-auth'
        }
      }
    }
  }
