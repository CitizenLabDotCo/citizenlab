...
module GemExtensions
  module Que
    module Migrations
      # This extension adapts Que's migrations to work with Apartment. The Que database
      # version (an internal version number used by Que to account for evolutions in how
      # it persists jobs) is stored as a comment on the `que_jobs` table. However, the
      # original implementation of `db_version` does not expect the database to contain
      # multiple `que_jobs` tables and as a result, is returning the version of one of
      # them indeterministically.
      module ClassMethods
        def db_version
