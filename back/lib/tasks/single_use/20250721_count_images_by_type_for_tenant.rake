...
namespace :single_use do
  ...
  task :count_images_by_type, %i[host date] => [:environment] do |_t, args|
    ...
    cutoff_date = args[:date] ? Date.parse(args[:date]) : Date.today
    ...
    Apartment::Tenant.switch(args[:host].tr('.', '_')) do
      ...
      content_builder_layout_images = ContentBuilder::LayoutImage.where('created_at < ?', cutoff_date).count
      puts "ContentBuilder::LayoutImage count: #{content_builder_layout_images}"
      total_images += content_builder_layout_images
      custom_field_option_images = CustomFieldOptionImage.where('created_at < ?', cutoff_date).count
      puts "CustomFieldOptionImage count: #{custom_field_option_images}"
      total_images += custom_field_option_images
      event_images = EventImage.where('created_at < ?', cutoff_date).count
      puts "EventImage count: #{event_images}"
      total_images += event_images
      idea_images = IdeaImage.where('created_at < ?', cutoff_date).count
      puts "IdeaImage count: #{idea_images}"
      total_images += idea_images
      total_possibly_user_images += idea_images
      project_folders_images = ProjectFolders::Image.where('created_at < ?', cutoff_date).count
      puts "ProjectFolders::Image count: #{project_folders_images}"
      total_images += project_folders_images
      project_images = ProjectImage.where('created_at < ?', cutoff_date).count
      puts "ProjectImage count: #{project_images}"
      total_images += project_images
      project_header_bg_images = Project.where('created_at < ?', cutoff_date).where.not(header_bg: nil).count
      puts "Project header background images count: #{project_header_bg_images}"
      total_images += project_header_bg_images
      project_folders_header_bg_images = ProjectFolders::Folder.where('created_at < ?', cutoff_date).where.not(header_bg: nil).count
      ...
      static_pages_header_bg_images = StaticPage.where('created_at < ?', cutoff_date).where.not(header_bg: nil).count
      puts "StaticPage header background images count: #{static_pages_header_bg_images}"
      total_images += static_pages_header_bg_images
      text_images = TextImage.where('created_at < ?', cutoff_date).count
      puts "TextImage count: #{text_images}"
      total_images += text_images
      total_possibly_user_images += text_images
      user_avatar_images = User.where('created_at < ?', cutoff_date).where.not(avatar: nil).count
      ...
      IdeaFile.all.each do |idea_file|
        ...
        idea_files_of_image_type += 1 if %w[jpg jpeg png gif tiff].include?(extension)
      end
      ...
      Files::File.all.each do |file|
        ...
        files_of_image_type += 1 if %w[jpg jpeg png gif tiff].include?(extension)
      end
      ...
      EventFile.all.each do |event_file|
        ...
        event_files_of_image_type += 1 if %w[jpg jpeg png gif tiff].include?(extension)
      end
      ...
      ProjectFile.all.each do |project_file|
        ...
        project_files_of_image_type += 1 if %w[jpg jpeg png gif tiff].include?(extension)
      end
      ...
      PhaseFile.all.each do |phase_file|
        ...
        phase_files_of_image_type += 1 if %w[jpg jpeg png gif tiff].include?(extension)
      end
      ...
      ProjectFolders::File.all.each do |project_folders_file|
        ...
        project_folders_files_of_image_type += 1 if %w[jpg jpeg png gif tiff].include?(extension)
      end
      ...
      StaticPageFile.all.each do |static_page_file|
        ...
        static_page_files_of_image_type += 1 if %w[jpg jpeg png gif tiff].include?(extension)
      end
      ...
      Files::FilesProject.all.each do |files_project|
        ...
        files_project_files_of_image_type += 1 if %w[jpg jpeg png gif tiff].include?(extension)
      end
