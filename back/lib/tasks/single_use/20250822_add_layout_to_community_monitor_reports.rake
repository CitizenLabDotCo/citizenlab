...
namespace :single_use do
  ...
  task :add_layout_to_community_monitor_reports, %i[execute] => [:environment] do |_t, args|
    ...
    Tenant.safe_switch_each do |tenant|
      ...
      community_monitor_autogenerated_reports = ReportBuilder::Report
        .where(
          phase_id: community_monitor_phase.id
        )
        .where.not(year: nil) # Only auto-generated reports have a year
      ...
      community_monitor_autogenerated_reports.each do |report|
        if report.layout.present?
          ...
          if execute
            ...
            print_padded "Added layout to report #{report.id}"
          else
            print_padded "Would add layout to report #{report.id}"
          end
        end
      end
    end
