...
describe EmailCampaigns::SideFxConsentService do
  ...
  describe 'after_create' do
    it "logs 'consent_given' when the created consent is consented" do
      consent = create(:consent, user: user, consented: true)
      expect { service.after_create(consent, user) }
        .to enqueue_job(LogActivityJob)
        .with(consent, 'consent_given', user, consent.updated_at.to_i, payload: { campaign_type: consent.campaign_type })
    end

    it "logs 'consent_withdrawn' when the created consent is not consented" do
  ...
  describe 'log_consent_event' do
    it 'logs a consent event even when consented did not change' do
      consent = create(:consent, user: user, consented: true)
      consent.reload
      expect { service.log_consent_event(consent, user) }
        .to enqueue_job(LogActivityJob)
