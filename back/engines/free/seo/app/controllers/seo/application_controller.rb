...
module Seo
  class ApplicationController < ActionController::Base
    ...
    def sitemap
      ...
      @projects = ProjectPolicy::Scope.new(nil, Project.all).resolve.select(
      ...
      @folders = ProjectFolders::FolderPolicy::Scope.new(nil, ProjectFolders::Folder.all).resolve.select(
      ...
      @ideas = IdeaPolicy::Scope.new(nil, Idea.all).resolve.select(:slug, :updated_at, :project_id).where(project_id: @projects.map(&:id))
    end
