...
resource 'Api Clients' do
  ...
  context 'as an admin' do
    ...
    get 'web_api/v1/api_clients' do
      ...
      let!(:api_client1) { create(:api_client, secret: '123456789abc') }
      let!(:api_client2) { create(:api_client) }

      example_request 'List all api clients' do
        expect(status).to eq 200

        expect(response_data.size).to eq 2
        expect(response_data_ids).to eq([api_client1.id, api_client2.id])
    ...
    get 'web_api/v1/api_clients/:id' do
      let(:api_client) { create(:api_client, secret: '123456789abc', last_used_at: Time.now) }
