...
module BulkImportIdeas
  class GoogleFormParserService
    ...
    def parse_paper_form
      ...
      # TODO: Not detecting the second pages of the document
      # TODO: Need to get the vector stuff properly so that the text appears in the right order
      # TODO: Checkboxes

      # Gets an array of documents and their fields
      document.entities.map do |entity|
        page_numbers = entity.page_anchor.page_refs.map(&:page)
        doc = {}
        document.pages.filter_map do |page|
          next unless page_numbers.include?(page.page_number - 1)

          page.form_fields.map do |field|
    ...
    def process_upload
      ...
      name = client.processor_path(
        project: '216810299084',
        location: 'us', # Invalid location: 'eu' must match the server deployment 'us' even when the processor is set to eu?
        processor: '9834e051aaf8e46a'
      )
