...
| attachments (space + proposal) | files |

`meetings`, `blogs`, `debates` and `awesome_iframe` components have no extractor and are ignored.
...
2. **`build_template`** — runs the extractors in dependency order so cross-record refs resolve:
   ...
   survey responses → static pages → files → description layouts. Each registers `Record`s in the
   `RefMap`.
