...
resource 'Map Layers' do
  ...
  shared_examples 'unauthorized POST, PATCH and DELETE map layer' do
    let(:map_config_id) { geojson_map_config.id }
    let(:id) { geojson_map_config.layers.first.id }

    post 'web_api/v1/map_configs/:map_config_id/layers', document: false do
      example_request '[unauthorized] Cannot create a map layer' do
        expect(status).to eq 401
      end
    end

    patch 'web_api/v1/map_configs/:map_config_id/layers/:id', document: false do
      example_request '[unauthorized] Cannot update a map layer' do
        expect(status).to eq 401
      end
    end

    patch 'web_api/v1/map_configs/:map_config_id/layers/:id/reorder', document: false do
      example_request '[unauthorized] Cannot reorder the layer(s)' do
        expect(status).to eq 401
      end
    end

    delete 'web_api/v1/map_configs/:map_config_id/layers/:id', document: false do
      example_request '[unauthorized] Cannot delete a map layer' do
        expect(status).to eq 401
      end
    end
  end

  shared_examples 'GET map GeoJSON layer' do
    get 'web_api/v1/map_configs/:map_config_id/layers/:id' do
      let(:map_config_id) { geojson_map_config.id }
      let(:layer) { geojson_map_config.layers.first }
      let(:id)    { layer.id }

      example_request 'Get a GeoJSON map layer' do
        assert_status 200
  ...
  shared_examples 'authorized POST GeoJSON map layer' do
    post 'web_api/v1/map_configs/:map_config_id/layers' do
  ...
  shared_examples 'authorized PATCH GeoJSON map layer' do
    patch 'web_api/v1/map_configs/:map_config_id/layers/:id' do
      with_options scope: :layer, required: true, with_example: true do
        parameter :title_multiloc,  'The name of the layer in multiple locales'
        parameter :type,            'The type of the layer (CustomMaps::GeojsonLayer or CustomMaps::EsriFeatureLayer)', required: true
  ...
  shared_examples 'GET an Esri feature layer' do
    get 'web_api/v1/map_configs/:map_config_id/layers/:id' do
      let(:map_config_id) { esri_map_config.id }
  ...
  shared_examples 'authorized POST Esri feature map layer' do
    post 'web_api/v1/map_configs/:map_config_id/layers' do
      with_options scope: :layer, required: true, with_example: true do
        parameter :title_multiloc,  'The name of the layer in multiple locales'
        parameter :type,            'The type of the layer (CustomMaps::GeojsonLayer or CustomMaps::EsriFeatureLayer)', required: true
  ...
  shared_examples 'authorized PATCH Esri feature map layer' do
    patch 'web_api/v1/map_configs/:map_config_id/layers/:id' do
      with_options scope: :layer, required: true, with_example: true do
        parameter :title_multiloc,  'The name of the layer in multiple locales'
        parameter :type,            'The type of the layer (CustomMaps::GeojsonLayer or CustomMaps::EsriFeatureLayer)', required: true
      ...
      let(:map_config_id) { esri_map_config.id }
      let(:id) { esri_map_config.layers.first.id }
  ...
  # --- common authorized examples ------

  shared_examples 'authorized reorder map layer' do
    patch 'web_api/v1/map_configs/:map_config_id/layers/:id/reorder' do
      with_options scope: :layer, required: true, with_example: true do
  ...
  shared_examples 'authorized DELETE map layer' do
    delete 'web_api/v1/map_configs/:map_config_id/layers/:id' do
      let(:map_config_id) { esri_map_config.id }
      let(:id) { esri_map_config.layers.first.id }
  ...
  context 'when not logged in' do
    include_examples 'GET map GeoJSON layer'
    include_examples 'GET an Esri feature layer'
  ...
  context 'when logged in as an admin' do
    ...
    include_examples 'GET map GeoJSON layer'
    include_examples 'authorized POST GeoJSON map layer'
    include_examples 'authorized PATCH GeoJSON map layer'

    include_examples 'GET an Esri feature layer'
    include_examples 'authorized POST Esri feature map layer'
    include_examples 'authorized PATCH Esri feature map layer'

    include_examples 'authorized reorder map layer'
    include_examples 'authorized DELETE map layer'
  end
