...
resource 'Map Configs' do
  explanation 'A map config defines how a custom_field map should be positioned and which layers it should display'

  let!(:custom_field) { create(:custom_field_point, resource: create(:custom_form)) }
  ...
  shared_examples 'successful GET map config' do
    get 'web_api/v1/custom_fields/:custom_field_id/map_config' do
      let!(:map_config) do
        create(:map_config,
          :with_positioning,
          :with_tile_provider,
      ...
      example_request 'Get the map config of a custom_field' do
  ...
  shared_examples 'unauthorized POST, PATCH and DELETE map config' do
    post 'web_api/v1/custom_fields/:custom_field_id/map_config' do
      example_request 'Cannot create a map config for a custom_field' do
        expect(status).to eq 401
      end
    end

    patch 'web_api/v1/custom_fields/:custom_field_id/map_config' do
      example_request 'Cannot update the map config for a custom_field' do
        expect(status).to eq 401
      end
    end

    delete 'web_api/v1/custom_fields/:custom_field_id/map_config' do
      example_request 'Cannot delete the map config for a custom_field' do
        expect(status).to eq 401
      end
    end
  end
  ...
  context 'when logged in as an admin' do
    ...
    patch 'web_api/v1/custom_fields/:custom_field_id/map_config' do
      ...
      context 'when the custom_field already has a map config' do
        before do
          create(:map_config,
            :with_positioning,
            :with_tile_provider,
      ...
      context 'when the custom_field does not have a map config' do
