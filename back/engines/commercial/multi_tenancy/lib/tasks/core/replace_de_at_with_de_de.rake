...
# Converts a single tenant from `de-AT` to `de-DE` in a single pass: adds
# `de-DE` to the tenant locales, rewrites every `de-AT` multiloc key (top-level
# or nested) to `de-DE`, migrates each `User.locale = 'de-AT'` to `'de-DE'`,
# and finally removes `de-AT` from the tenant locales.
#
# What it does:
#   - Targets one tenant, identified by the `host` argument.
#   - Ensures `de-DE` is in the tenant's `core.locales` settings (adds if missing).
#   - For every JSONB column on every ApplicationRecord model, recursively
#     walks the value and, for every hash where `de-AT` is a key AND `de-DE`
#     is absent or empty, moves the `de-AT` value to a `de-DE` key (deleting
#     `de-AT`). This catches both top-level multiloc columns (e.g.
#     `title_multiloc`) and nested multilocs embedded in larger JSON blobs
#     (e.g. `ContentBuilder::Layout#craftjs_json`).
#   - Updates every `User` whose `locale` is `'de-AT'` to `'de-DE'`.
#   - Removes `de-AT` from the tenant's `core.locales` settings (if present).
#   - Writes a JSON report with the before/after JSON values and the paths
#     that were rewritten.
#
# What it does NOT do:
#   - It does not check for the presence of `de-AT` in the tenant locales before
#     starting, as this task is designed to also fix cases where `de-AT` has
#     been mistakenly removed before running this task. A warning is printed.
#   - It does not translate content - the `de-DE` value is an exact copy of the
#     `de-AT` value.
#   - It does not touch a multiloc that already has a populated `de-DE` value
#     at the same path; such cases are reported as skipped and the `de-AT`
...
#       cl2back:replace_de_AT_with_de_DE[example.com]
#
#   Execute (applies the changes):
#     docker compose run --rm web bundle exec rake \
#       cl2back:replace_de_AT_with_de_DE[example.com,execute]

namespace :cl2back do
  desc 'For a tenant, rename de-AT multiloc keys to de-DE across every model, and update tenant locales accordingly.'
  task :replace_de_AT_with_de_DE, %i[host execute] => [:environment] do |_t, args|
    host = args[:host]
    execute = args[:execute] == 'execute'

    puts "---------- STARTING TASK: Replace de-AT with de-DE ----------\n\n"
    puts "Mode: #{execute ? 'EXECUTE - changes WILL be applied' : 'Dry run - no changes will be applied'}\n\n"

    reporter = ScriptReporter.new

    if host.blank?
      puts 'ERROR! No host argument provided. Usage: rake cl2back:replace_de_AT_with_de_DE[example.com,execute]'
      next
    end
    ...
    tenant.switch do
      ...
      # de-AT locale has ben mistakenly removed before running this task.
      puts "NO de-AT LOCALE FOUND IN TENANT LOCALES!.\n\n" unless locales_before.include?('de-AT')

      # 1) Add de-DE to tenant locales if missing.
      if locales_before.include?('de-DE')
        puts "Tenant locales already include 'de-DE' - no change needed.\n\n"
      else
        puts "#{execute ? 'ADDING' : 'WOULD ADD'} 'de-DE' to tenant locales.\n\n"
        if execute
      ...
      # `de-AT` hash keys at any nesting depth and rewrites them to `de-DE`
      # (when `de-DE` is absent or empty at that path).
      ...
      models.sort_by(&:name).each do |model|
        ...
        json_columns.each do |column|
          ...
          # contains the literal "de-AT" anywhere. The walker then verifies
          # whether each occurrence is a real hash key.
          scope = model.where("#{attr}::text LIKE ?", '%"de-AT"%')
          ...
          scope.find_each do |record|
            ...
            walker = ReplaceDeAtWithDeDeWalker.new
            after = original.deep_dup
            walker.process!(after)

            walker.skipped_paths.each do |p|
              puts "  SKIP: #{model.name} #{record.id} #{attr} at #{p} (de-DE already populated)"
            end
      ...
      # 2b) Migrate User.locale = 'de-AT' -> 'de-DE'.
      users_scope = User.where(locale: 'de-AT')
      user_count = users_scope.count
      total_users_updated = 0

      if user_count.positive?
        puts "\nUser#locale: #{user_count} user(s) with locale 'de-AT'"
        users_scope.find_each do |user|
          context = { tenant: host, model: 'User', record_id: user.id, attribute: 'locale' }
          reporter.add_change({ 'locale' => 'de-AT' }, { 'locale' => 'de-DE' }, context: context)
          total_users_updated += 1
          puts "  #{execute ? 'REPLACED' : 'WOULD REPLACE'} User #{user.id} locale 'de-AT' -> 'de-DE'"

          next unless execute

          begin
            user.update_columns(locale: 'de-DE')
          rescue StandardError => e
            reporter.add_error(e.message, context: context)
      ...
      # 3) Remove de-AT from tenant locales if present.
      app_config.reload
      locales_now = app_config.settings('core', 'locales') || []
      if locales_now.include?('de-AT')
        puts "\n#{execute ? 'REMOVING' : 'WOULD REMOVE'} 'de-AT' from tenant locales."
        if execute
          settings = app_config.settings
        ...
        puts "\nTenant locales do not include 'de-AT' - no removal needed."
      end
      ...
      puts "  Multiloc keys skipped (de-DE already populated):  #{total_skipped}"
      puts "  User.locale values #{execute ? 'replaced' : 'to be replaced'}:          #{total_users_updated}"
    end

    begin
      reporter.report!('replace_de_AT_with_de_DE.json', verbose: false)
      puts "\nReport written to replace_de_AT_with_de_DE.json"
    rescue StandardError => e
      puts "ERROR! Failed to write report: #{e.message}"
    end

    puts "\n---------- FINISHED TASK: Replace de-AT with de-DE ----------\n\n"
  end
end

# Recursively walks a JSON structure and renames every `de-AT` hash key to
# `de-DE`, keeping a duplicate of the value. A multiloc that already has a
# populated (non-empty) `de-DE` value at the same path is left untouched.
# Renames hash keys only; string values are not touched.
class ReplaceDeAtWithDeDeWalker
  ...
  def walk(node, path)
    ...
    when Hash
      ...
      replace_de_at_with_de_de_key(node, path) if node.key?('de-AT')
    when Array
      node.each_with_index { |value, index| walk(value, "#{path}[#{index}]") }
    end
  end

  def replace_de_at_with_de_de_key(multiloc, path)
    de_de_value = multiloc['de-DE']
    # Replace if de-DE is absent (or JSON null) or an empty string; otherwise
    # leave de-AT alone.
    if de_de_value.nil? || de_de_value == ''
      multiloc['de-DE'] = multiloc.delete('de-AT')
      @replaced += 1
      @replaced_paths << path
    else
      @skipped += 1
