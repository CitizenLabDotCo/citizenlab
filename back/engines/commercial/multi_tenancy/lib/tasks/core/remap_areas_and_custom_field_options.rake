...
# Usage:
# rails fix_existing_tenants:remap_areas_and_custom_field_options[doemee.inulst.nl,tmp/mapping.csv,nl-NL]
#
...
namespace :fix_existing_tenants do
  ...
  task :remap_areas_and_custom_field_options, %i[host csv_path locale] => :environment do |_task, args|
    ...
    raise 'Please provide csv_path argument' if csv_path.blank?
    ...
    tenant.switch do
      puts "Running for tenant: #{tenant.host}"
      RemapAreasTask.run(csv_path:, locale:)
...
module RemapAreasTask
  module_function

  def run(csv_path:, locale:)
    ...
    ActiveRecord::Base.transaction do
      target_groups.each do |target_name, old_new_pairs|
        process_target_group(target_name, old_new_pairs, domicile_field)
      end
  ...
  def process_target_group(target_name, old_new_pairs, domicile_field)
    ...
    if matching_areas.size == 1 && existing_target_area.nil?
      process_simple_rename(matching_areas.first, target_name)
    else
      process_merge(target_name, matching_areas, existing_target_area, domicile_field)
    end
  end

  def process_simple_rename(area, target_name)
    default_locale = I18n.default_locale.to_s
  ...
  def process_merge(target_name, matching_areas, existing_target_area, domicile_field)
    default_locale = I18n.default_locale.to_s
