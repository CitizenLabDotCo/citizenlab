...
module CustomIdMethods::NemlogIn
  class NemlogInOmniauth < IdMethods::Base
    ...
    ENVIRONMENTS = {
      ...
      production: {
        metadata_xml_file: File.join(CustomIdMethods::Engine.root, 'config', 'saml', 'nemlog_in', 'idp_metadata', 'production.xml')
      }
    }.freeze
    ...
    def fetch_municipality_code(cpr_number)
      CustomIdMethods::NemlogIn::KkiLocationApi.new.municipality_code(cpr_number)
    end
