...
resource "Verifications" do
  ...
  before do
    ...
    settings['verification'] = {
      allowed: true,
      enabled: true,
      verification_methods: [{ name: 'GentRrn', api_username: 'fake_username', api_password: 'fake_password', rut_empresa: 'fake_rut_empresa' }],
    }
    configuration.save!
  end

  post "web_api/v1/verification_methods/GentRrn/verification" do
    ...
    before do
      stub_request(:get, "https://terceros.sidiv.registrocivil.cl:8443/InteroperabilityPlateform/TercerosGentRrnProxyService?wsdl")
        .to_return(status: 200, body: File.read("engines/commercial/id_gent_rrn/spec/fixtures/GentRrn_wsdl.xml"), headers: {})
      savon.mock!
    end
    after do
      savon.unmock!
    ...
    #   example_request "Verify with GentRrn for real" do
    ...
    describe do
      ...
      example "Verify with GentRrn" do
        savon.expects(:get_data_document)
             ...
             .returns(File.read("engines/commercial/id_gent_rrn/spec/fixtures/get_data_document_match.xml"))
        do_request
        expect(status).to eq(201)
        expect(@user.reload.verified).to be true
        expect(@user.verifications.first).to have_attributes({
    ...
    describe do
      ...
      let(:id_serial) { "A001529382" }
      example "[error] Verify with GentRrn without a match" do
        savon.expects(:get_data_document)
             .with(message: {
               "typens:RUTEmpresa" => 'fake_rut_empresa',
               "typens:DVEmpresa" => 'k',
             ...
             .returns(File.read("engines/commercial/id_gent_rrn/spec/fixtures/get_data_document_no_match.xml"))
        do_request
    ...
    describe do
      ...
      example "[error] Verify with GentRrn with a match that's not entitled to verification" do
        savon.expects(:get_data_document)
             .with(message: {
               "typens:RUTEmpresa" => 'fake_rut_empresa',
               "typens:DVEmpresa" => 'k',
             ...
             .returns(File.read("engines/commercial/id_gent_rrn/spec/fixtures/get_data_document_match_no_citizen.xml"))
        do_request
        expect(status).to eq (422)
        json_response = json_parse(response_body)
    ...
    describe do
      ...
      let(:id_serial) { "A001529382" }
      example_request "[error] Verify with GentRrn using invalid run" do
        expect(status).to eq (422)
        json_response = json_parse(response_body)
    ...
    describe do
      let(:run) { "12.025.365-6" }
      let(:id_serial) { "" }
      example_request "[error] Verify with GentRrn using invalid id_serial" do
        expect(status).to eq (422)
        json_response = json_parse(response_body)
    ...
    describe do
      ...
      example "[error] Verify with GentRrn using credentials that are already taken" do
        savon.expects(:get_data_document)
             .with(message: :any)
             .returns(File.read("engines/commercial/id_gent_rrn/spec/fixtures/get_data_document_match.xml"))
        do_request
