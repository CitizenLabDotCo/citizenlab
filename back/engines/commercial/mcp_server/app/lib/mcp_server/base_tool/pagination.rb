...
module McpServer::BaseTool::Pagination
  ...
  def paginated_response(label, scope, page: nil, per_page: nil, serializer: nil, **json_options)
    page ||= 1
    ...
    data = serializer ? serializer.serialize_all(records) : records.as_json(**json_options)
