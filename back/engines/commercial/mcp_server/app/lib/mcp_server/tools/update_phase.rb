...
class McpServer::Tools::UpdatePhase < McpServer::BaseTool
  ...
  class Runner < McpServer::BaseTool::Runner
    def run
      attributes = params.except(:phase_id).symbolize_keys
      rejected = attributes.keys - updatable_attributes
      return error("These fields can't be updated on a phase: #{rejected.join(', ')}.") if rejected.any?
      ...
      apply_attributes(phase, attributes)
    ...
    def updatable_attributes
      McpServer::Tools::UpdatePhase.new.input_schema[:properties].keys.map(&:to_sym) - [:phase_id]
    end
  end
end
