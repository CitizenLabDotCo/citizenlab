...
resource 'Analytics - FactEmailDeliveries model' do
  ...
  post 'web_api/v1/analytics' do
    ...
    example 'count sent email deliveries' do
      ...
      assert_status 200
      expect(response_data[:data][:attributes]).to eq([{ count: 4 }])
    end
    ...
    example 'count custom email deliveries' do
      ...
      assert_status 200
      expect(response_data[:data][:attributes]).to eq([{ count: 1 }])
    end
    ...
    example 'count email campaigns' do
      ...
      assert_status 200
      expect(response_data[:data][:attributes]).to eq([{ count_campaign_id: 3 }])
    end
  end
end
