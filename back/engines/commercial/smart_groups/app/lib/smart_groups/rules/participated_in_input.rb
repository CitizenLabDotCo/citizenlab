...
module SmartGroups::Rules
  class ParticipatedInInput
      ...
      [
        ...
        },
        {
          ...
          'properties' => {
            ...
            'value' => {
              'description' => 'The ids of some inputs',
              'type' => 'array',
              'items' => {
                'type' => 'string'
              },
    ...
    def filter(users_scope)
      ...
      case predicate
      when 'in'
        participants = participants_service.ideas_participants(Idea.where(id: value))
        users_scope.where(id: participants)
      when 'not_in'
        participants = participants_service.ideas_participants(Idea.where(id: value))
        users_scope.where.not(id: participants)
      when 'posted_in'
        participants = participants_service.ideas_participants(Idea.where(id: value), actions: [:posting])
        users_scope.where(id: participants)
      when 'not_posted_in'
        participants = participants_service.ideas_participants(Idea.where(id: value), actions: [:posting])
        users_scope.where.not(id: participants)
      when 'commented_in'
        participants = participants_service.ideas_participants(Idea.where(id: value), actions: [:commenting])
        users_scope.where(id: participants)
      when 'not_commented_in'
        participants = participants_service.ideas_participants(Idea.where(id: value), actions: [:commenting])
        users_scope.where.not(id: participants)
      when 'reacted_idea_in'
        participants = participants_service.ideas_participants(Idea.where(id: value), actions: [:idea_reacting])
        users_scope.where(id: participants)
      when 'not_reacted_idea_in'
        participants = participants_service.ideas_participants(Idea.where(id: value), actions: [:idea_reacting])
        users_scope.where.not(id: participants)
      when 'reacted_comment_in'
        participants = participants_service.ideas_participants(Idea.where(id: value), actions: [:comment_reacting])
        users_scope.where(id: participants)
    ...
    def description_value(locale)
      if multivalue_predicate?
        value.map do |v|
          Idea.find(v).title_multiloc[locale]
        end.join ', '
      else
        Idea.find(value).title_multiloc[locale]
      end
    end

    def value_in_inputs
      if multivalue_predicate?
        errors.add(:value, :has_invalid_input) unless value.is_a?(Array) && Idea.where(id: value).count == value.size
      else
        errors.add(:value, :has_invalid_input) unless value.is_a?(String) && Idea.exists?(id: value)
      end
    end
  end
end
