...
module SmartGroups::Rules
  class ParticipatedInTopic
    ...
    def filter(users_scope)
      ...
      case predicate
      when 'in'
        participants = participants_service.topics_participants(GlobalTopic.where(id: value))
        users_scope.where(id: participants)
      when 'not_in'
        participants = participants_service.topics_participants(GlobalTopic.where(id: value))
        users_scope.where.not(id: participants)
      when 'posted_in'
        participants = participants_service.topics_participants(GlobalTopic.where(id: value), actions: [:posting])
        users_scope.where(id: participants)
      when 'not_posted_in'
        participants = participants_service.topics_participants(GlobalTopic.where(id: value), actions: [:posting])
        users_scope.where.not(id: participants)
      when 'commented_in'
        participants = participants_service.topics_participants(GlobalTopic.where(id: value), actions: [:commenting])
        users_scope.where(id: participants)
      when 'not_commented_in'
        participants = participants_service.topics_participants(GlobalTopic.where(id: value), actions: [:commenting])
        users_scope.where.not(id: participants)
      when 'reacted_idea_in'
        participants = participants_service.topics_participants(GlobalTopic.where(id: value), actions: [:idea_reacting])
        users_scope.where(id: participants)
      when 'not_reacted_idea_in'
        participants = participants_service.topics_participants(GlobalTopic.where(id: value), actions: [:idea_reacting])
        users_scope.where.not(id: participants)
      when 'reacted_comment_in'
        participants = participants_service.topics_participants(GlobalTopic.where(id: value), actions: [:comment_reacting])
        users_scope.where(id: participants)
    ...
    def description_value(locale)
      if multivalue_predicate?
        value.map do |v|
          GlobalTopic.find(v).title_multiloc[locale]
        end.join ', '
      else
        GlobalTopic.find(value).title_multiloc[locale]
      end
    end
    ...
    def value_in_topics
      if multivalue_predicate?
        errors.add(:value, :has_invalid_topic) unless (value - GlobalTopic.ids).empty?
      else
        errors.add(:value, :has_invalid_topic) unless GlobalTopic.ids.include?(value)
      end
    end
  end
end
