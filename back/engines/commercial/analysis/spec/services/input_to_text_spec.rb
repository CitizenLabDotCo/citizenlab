...
describe Analysis::InputToText do
  describe '#execute' do
    it 'works with ideation built-in textual fields' do
      ...
      custom_fields = custom_form.custom_fields.filter { |field| field.input_strategy.supports_free_text_value? }
    ...
    it 'truncates long values when passed the `truncate_values` option' do
      custom_form = create(:custom_form, :with_default_fields)
      service = described_class.new(custom_form.custom_fields.filter { |field| field.input_strategy.supports_free_text_value? })
