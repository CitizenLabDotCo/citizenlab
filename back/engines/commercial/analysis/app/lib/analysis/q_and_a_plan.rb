...
module Analysis
  # The plan is the output of the #plan method on the q&a method. It
  # describes how the q&a should be done, mainly what method and which
  # LLM to use
  class QAndAPlan
    include ActiveModel::API
    IMPOSSIBLE_REASONS = [:too_many_inputs]

    attr_accessor :q_and_a_method_class, :llm, :truncate_values, :include_id, :shorten_labels, :accuracy, :impossible_reason, :include_comments
    ...
    with_options unless: -> { impossible_reason } do
      validates :q_and_a_method_class, inclusion: { in: QAndAMethod::Base::Q_AND_A_METHOD_CLASSES }
      validates :llm, inclusion: { in: QAndAMethod::Base::LLMS }
