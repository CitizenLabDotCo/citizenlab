...
describe WebApi::V1::IdeaSerializer do
  context 'custom_field_values' do
    it 'leaves out disabled fields' do
      SettingsService.new.activate_feature! 'idea_custom_fields'

      project = create :project
      form = create :custom_form, project: project
      visible_field = create :custom_field, :for_custom_form, resource: form, enabled: true, input_type: 'number'
      disabled_field = create :custom_field, :for_custom_form, resource: form, enabled: false, input_type: 'number'
      custom_field_values = { visible_field.key => 1, disabled_field.key => 2 }
