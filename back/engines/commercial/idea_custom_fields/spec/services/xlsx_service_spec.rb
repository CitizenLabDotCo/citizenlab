    ...
    context 'when a project has no custom form - and therefore no fields' do
      ...
      it 'does not add any custom_fields' do
        ...
        expect(headers).to match_array(
          %w[id title description proposed_budget published_at comments likes dislikes url project topics status latitude longitude location_description attachments]
        )
