namespace :fix_existing_tenants do
  ...
  task :migrate_initialization_finished_at, [] => [:environment] do
    Activity.where(action: 'template_loaded', item_type: 'Tenant').each do |activity|
      tenant = activity.item
      tenant.update! initialization_finished_at: activity.acted_at if !tenant.deleted_at
    end
    Tenant.not_deleted.where('created_at < ?', Time.parse('01-01-2021')).each do |tenant|
      tenant.initialization_finished_at ||= tenant.created_at
      tenant.save!
    end
  end
end
