  ...
  describe 'after_update' do
    it "logs a 'changed' action job when the configuration has changed" do
      ...
      expect { service.after_update(config, current_user) }
        ...
        .and have_enqueued_job(LogActivityJob).with(current_tenant, 'changed', current_user, config.updated_at.to_i, {}) # TODO_MT to be removed
    end
    ...
    it "logs a 'changed_lifecycle_stage' action job when the lifecycle has changed" do
      ...
      expect { service.after_update(config, current_user) }
        .to  have_enqueued_job(LogActivityJob).with(config, 'changed_lifecycle_stage', current_user, updated_at,
          options)
        .and have_enqueued_job(LogActivityJob).with(current_tenant, 'changed_lifecycle_stage', current_user, updated_at, options) # TODO_MT to be removed
    end

    context 'when tenant host is changed' do
      # Avoid problems with CI failures, where the host change applied in one of the tests here seems to
      # persist and break subsequent tests (on CI), by using a separate tenant here and then switching
