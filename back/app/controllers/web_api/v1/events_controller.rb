...
class WebApi::V1::EventsController < ApplicationController
  ...
  def attendees_xlsx
    ...
    I18n.with_locale(current_user&.locale) do
      xlsx = Export::Xlsx::AttendeesGenerator.new.generate_attendees_xlsx attendees.includes(:custom_field_answers), view_private_attributes: true
  ...
  def apply_projects_listed_scope(events, params)
    ...
    # Same when filtering by attendee_id: the scope is already limited to that user's
    # own registrations, and EventPolicy::Scope has checked the caller may see them
