...
class WebApi::V1::PhasesController < ApplicationController
  ...
  def phase_params
    ...
    params.require(:phase).permit(permitted).tap do |attrs|
      attrs[:start_at] = Time.zone.parse(attrs[:start_at]) if date_format?(attrs[:start_at])
      attrs[:end_at] = Time.zone.parse(attrs[:end_at]) + 1.day if date_format?(attrs[:end_at])
    end
  end
