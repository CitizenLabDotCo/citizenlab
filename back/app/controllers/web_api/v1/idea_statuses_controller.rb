...
class WebApi::V1::IdeaStatusesController < ApplicationController
  ...
  def apply_index_filters
    ...
    unless prescreening_feature_active?(participation_method)
      @idea_statuses = @idea_statuses.where.not(code: 'prescreening')
    end
  end

  def prescreening_feature_active?(participation_method)
    (participation_method == 'ideation' && AppConfiguration.instance.feature_activated?('prescreening_ideation')) ||
