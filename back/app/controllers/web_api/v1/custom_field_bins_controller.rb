...
class WebApi::V1::CustomFieldBinsController < ApplicationController
  ...
  def index
    bins = @custom_field.custom_field_bins.includes(:custom_field_option)
    bins = bins.order(:created_at)
    bins = paginate(bins)
    render json: linked_json(bins, WebApi::V1::CustomFieldBinSerializer, params: jsonapi_serializer_params, include: [:custom_field_option])
  end
