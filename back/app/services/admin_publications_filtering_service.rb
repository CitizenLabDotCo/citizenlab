...
class AdminPublicationsFilteringService
  ...
  add_filter('remove_not_allowed_parents') do |scope, options|
    ...
    parents_with_visible_children_publication_ids = AdminPublication.includes(:parent)
      .where(publication_id: Project.publicly_visible.not_draft.ids.uniq)
      .where.not(parent_id: nil)
      .map(&:parent_id)

    parents_with_visible_children = scope.where(id: parents_with_visible_children_publication_ids)
