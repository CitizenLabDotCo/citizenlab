  ...
  module Xlsx
    class InputSheetGenerator
      ...
      def created_at_report_field
        ComputedFieldForReport.new(column_header_for('created_at'), ->(input) { input.created_at&.in_time_zone(AppConfiguration.timezone) })
      end

      def published_at_report_field
        ComputedFieldForReport.new(column_header_for('published_at'), ->(input) { input.published_at&.in_time_zone(AppConfiguration.timezone) })
      end
