class SideFxOfficialFeedbackService
  ...
  def after_create feedback, user
    LogActivityService.new.run(feedback, 'created', user, feedback.created_at.to_i)
  ...
  def after_update feedback, user
    LogActivityService.new.run(feedback, 'changed', user, feedback.updated_at.to_i)
  ...
  def after_destroy frozen_feedback, user
    ...
    LogActivityService.new.run(encode_frozen_resource(frozen_feedback), 'deleted', user, Time.now.to_i, payload: {official_feedback: serialized_feedback})
  end
  ...
  def notify_mentioned_users feedback, user
    ...
    mentioned_users.uniq.each do |mentioned_user|
      LogActivityService.new.run(feedback, 'mentioned', user, feedback.created_at.to_i, payload: {mentioned_user: mentioned_user.id})
    end
  end
