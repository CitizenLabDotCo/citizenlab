...
module ProjectFolders
  class SideFxService
    ...
    def after_create(folder, user)
      folder.update!(description_multiloc: TextImageService.new.swap_data_images_multiloc(folder.description_multiloc, :description_multiloc))
      LogActivityJob.perform_later(folder, 'created', user, folder.created_at.to_i)
    end

    def before_update(folder, _user)
      folder.description_multiloc = TextImageService.new.swap_data_images_multiloc(folder.description_multiloc, :description_multiloc)
    end
