class SideFxProjectsTopicService
  ...
  def after_create projects_topic, user
    LogActivityService.new.run(projects_topic, 'created', user, projects_topic.created_at.to_i)
  end
  ...
  def after_update projects_topic, user
    LogActivityService.new.run(projects_topic, 'changed', user, projects_topic.updated_at.to_i)
  end
  ...
  def after_destroy frozen_projects_topic, user
    ...
    LogActivityService.new.run(encode_frozen_resource(frozen_projects_topic), 'deleted', user, Time.now.to_i, payload: {projects_topic: serialized_projects_topic})
  end
