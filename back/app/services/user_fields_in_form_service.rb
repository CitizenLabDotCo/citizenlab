...
class UserFieldsInFormService
  ...
  class << self
    ...
    def should_merge_user_fields_into_idea?(
      current_user,
      phase,
      idea
    )
    ...
    def merge_user_fields_into_idea(
      current_user,
      phase,
    ...
    def should_merge_user_fields_from_idea_into_user?(idea, user, phase)
    ...
    def add_user_fields_to_form(fields, participation_method, custom_form)
    ...
    def user_fields_in_form_descriptor_ideationlike(permission)
      if permission.permitted_by == 'everyone'
        {
          value: true,
          locked: true,
