class SideFxInitiativeService
  ...
  def log_activity_jobs_after_published initiative, user
    LogActivityService.new.run(initiative, 'published', user, initiative.published_at.to_i)
    if first_user_initiative? initiative, user
      LogActivityService.new.run(initiative, 'first_published_by_user', user, initiative.published_at.to_i)
    end
  end
