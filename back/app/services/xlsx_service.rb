...
class XlsxService
  ...
  def xlsx_to_hash_array(xlsx)
    ...
    worksheet.drop(1).map do |row|
      xlsx_utils = Export::XlsxExport::Utils.new
  ...
  def generate_idea_xlsx_columns(_ideas, view_private_attributes: false, with_tags: false)
    columns = [
      ...
      { header: 'description',          f: ->(i) { Export::XlsxExport::Utils.new.convert_to_text_long_lines(multiloc_service.t(i.body_multiloc)) }, width: 10 },
  ...
  def generate_idea_comments_xlsx(comments, view_private_attributes: false)
    columns = [
      ...
      { header: 'comment',            f: ->(c) { Export::XlsxExport::Utils.new.convert_to_text_long_lines(multiloc_service.t(c.body_multiloc)) }, width: 10 },
  ...
  def remove_duplicate_header_suffix(headers)
    headers.map do |header|
      Export::XlsxExport::Utils.new.remove_duplicate_column_name_suffix header
    end
  end
end
