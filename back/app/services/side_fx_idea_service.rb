...
class SideFxIdeaService
  ...
  def after_create(idea, user)
    idea.update!(body_multiloc: TextImageService.new.swap_data_images_multiloc(idea.body_multiloc, :body_multiloc))
    after_publish idea, user if idea.published?
  end

  def before_update(idea, user)
    idea.body_multiloc = TextImageService.new.swap_data_images_multiloc(idea.body_multiloc, :body_multiloc)
