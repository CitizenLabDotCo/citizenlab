class SideFxIdeaService
  ...
  def log_activity_jobs_after_published(idea, user)
    LogActivityService.new.run(idea, 'published', user, idea.published_at.to_i)
    scrape_facebook(idea)
    return unless first_user_idea?(idea, user)

    LogActivityService.new.run(idea, 'first_published_by_user', user, idea.published_at.to_i)
  end
