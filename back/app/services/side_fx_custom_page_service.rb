...
class SideFxCustomPageService
  include SideFxHelper

  def before_create(page, user); end

  def after_create(page, user)
    page.update! bottom_info_section_multiloc: TextImageService.new.swap_data_images(page, :bottom_info_section_multiloc)
  ...
  def after_destroy(frozen_page, user)
    ...
    LogActivityJob.perform_later(
      ...
      user, Time.now.to_i,
      payload: { custom_page: serialized_page }
    )
  end
end
