...
--   as the timeline phase whose [start_at, end_at) window contains
--   contributed_at. Timeline phases of a project never overlap. Standalone
--   phases may overlap anything and are excluded from inference: their
--   contributions always carry a creation phase, so they are attributed
--   structurally.
...
--   of the timeline phase active at action time, falling back to the input's
...
LEFT JOIN phases inferred_ph
    ...
    AND inferred_ph.placement_type = 'on_timeline'
