...
describe 'single_use:reset_users_in_email_limbo rake task' do
  ...
  context 'dry-run mode' do
    it 'does not modify users in email limbo' do
      ...
      expect(unconfirmed_user.reload.email).not_to be_nil
      expect(full_user.reload.email).not_to be_nil
      expect(full_user_with_identity.reload.email).not_to be_nil
      expect(full_user_with_identity_and_unconfirmed_email.reload.email).not_to be_nil
    end
  end

  context 'execute mode' do
    it 'resets email for users in email limbo' do
      run_task(execute: true)

      expect(unconfirmed_user.reload.email).not_to be_nil
      expect(full_user.reload.email).not_to be_nil
      expect(full_user_with_identity.reload.email).not_to be_nil
      expect(full_user_with_identity_and_unconfirmed_email.reload.email).to be_nil
    end
  end
end
