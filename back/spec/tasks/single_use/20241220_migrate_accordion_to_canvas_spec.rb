...
RSpec.describe 'Accordion migration rake task', type: :task do
  ...
  describe 'migration' do
    ...
    it 'migrates accordion with text content to canvas mode' do
      ...
      expect(accordion_node['nodes'].length).to eq(1)
  ...
  describe 'test migration' do
    ...
    it 'analyzes migration without making changes' do
      ...
      expect(layout.reload.craftjs_json['ACCORDION_1']['props']['isCanvas']).to be_nil
      expect(layout.reload.craftjs_json['ACCORDION_2']['props']['isCanvas']).to be_nil
    end
  end
end
