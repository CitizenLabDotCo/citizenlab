...
describe 'single_use:update_rights_given_activities rake task' do
  ...
  let!(:activity4) { create(:activity, action: 'project_moderation_rights_received') }
  let!(:activity5) { create(:activity, action: 'folder_moderation_rights_received') }
  ...
  it 'updates the action of the relevant activities' do
    ...
    expect(activity4.reload.action).to eq('project_moderation_rights_received')
    expect(activity5.reload.action).to eq('folder_moderation_rights_received')
  end
end
