...
describe 'single_use:migrate_voting_phases_off_feed_view rake task' do
  before { load_rake_tasks_if_not_loaded }

  after do
    Rake::Task['single_use:migrate_voting_phases_off_feed_view'].reenable
    FileUtils.rm_f(report_path)
    FileUtils.rm_f(dry_run_report_path)
  end

  let(:report_path) { Rails.root.join('migrate_voting_phases_off_feed_view.json') }
  let(:dry_run_report_path) { Rails.root.join('migrate_voting_phases_off_feed_view_dry_run.json') }
  ...
  def run_task(dry_run: false, host: nil)
    Rake::Task['single_use:migrate_voting_phases_off_feed_view'].invoke(dry_run ? nil : 'execute', host)
  end

  # The feed view is withdrawn for voting, so the model refuses to build this state. Write it past
  # validation, the way the REST API and the MCP tools could before the accompanying rule landed.
  ...
  context 'when a voting phase opens on the feed view' do
    # `:ongoing` so the timing the task derives is 'active'; the factory's own dates are historic.
    let!(:phase) { offer_feed!(create(:single_voting_phase, :ongoing)) }
  ...
  context 'when a voting phase offers the feed view but opens on another one' do
    let!(:phase) do
      offer_feed!(create(:single_voting_phase), presentation_mode: 'map', available_views: %w[card map feed])
    end
  ...
  context 'when a phase opens on a view it does not offer' do
    let!(:phase) do
      offer_feed!(create(:single_voting_phase), presentation_mode: 'map', available_views: %w[card feed])
    end
  ...
  context 'when a phase using the feed view also omits the card view' do
    let!(:phase) { offer_feed!(create(:single_voting_phase), available_views: ['feed']) }
  ...
  context 'when a voting phase does not use the feed view' do
    let!(:phase) { create(:budgeting_phase, presentation_mode: 'map', available_views: %w[card map]) }
  ...
  context 'when the task is run twice' do
    let!(:phase) { offer_feed!(create(:single_voting_phase)) }
