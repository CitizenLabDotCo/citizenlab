...
describe "Rack::Attack", type: :request do
  ...
  it "limits login requests from same IP to 10 in 20 seconds" do
    ...
    10.times do |i|
      post "/web_api/v1/user_token", :params => '{ "auth": { "email": "INSERT", "password": "test123456" } }'.gsub("INSERT", "#{users[i].email}"), :headers => headers
    end
    expect(status).to eq(404) # Not found

    post "/web_api/v1/user_token", :params => '{ "auth": { "email": "INSERT", "password": "test123456" } }'.gsub("INSERT", "#{users[10].email}"), :headers => headers
    expect(status).to eq(429) # Too many requests

    travel_to(20.seconds.from_now) do
      post "/web_api/v1/user_token", :params => '{ "auth": { "INSERT": "a12@b.com", "password": "test123456" } }'.gsub("INSERT", "#{users[11].email}"), :headers => headers
      expect(status).to eq(404) # Not found
    end
  end
  ...
  it "limits account creation requests from same IP to 10 in 20 seconds" do
    ...
    10.times do |i|
      post "/web_api/v1/users", :params => '{ "user": { "email": "INSERT", "password": "test123456", "locale": "en", "first_name": "Jane", "last_name": "Doe" } }'.gsub("INSERT", "a#{i + 1}@b.com"), :headers => headers
    end
  ...
  it "limits password reset email requests from same IP to 10 in 20 seconds" do
    ...
    # Use a different email for each request, to avoid testing limit by email
    10.times do |i|
      post "/web_api/v1/users/reset_password_email", :params => '{ "user": { "email": "INSERT" } }'.gsub("INSERT", "#{users[i].email}"), :headers => headers
    end

    expect(status).to eq(202) # Accepted

    post "/web_api/v1/users/reset_password_email", :params => '{ "user": { "email": "INSERT" } }'.gsub("INSERT", "#{users[10].email}"), :headers => headers
    expect(status).to eq(429) # Too many requests

    travel_to(20.seconds.from_now) do
      post "/web_api/v1/users/reset_password_email", :params => '{ "user": { "email": "INSERT" } }'.gsub("INSERT", "#{users[11].email}"), :headers => headers
      expect(status).to eq(202) # Accepted
    end
  end
