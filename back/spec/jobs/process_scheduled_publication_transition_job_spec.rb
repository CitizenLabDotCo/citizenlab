...
RSpec.describe ProcessScheduledPublicationTransitionJob do
  ...
  describe '#run' do
    context 'happy path' do
      before do
        admin_publication.update_columns(scheduled_status: 'published', scheduled_at: 1.hour.ago)
      end

      it 'materializes the transition and clears scheduled fields' do
        job.run(admin_publication.id, user.id)
      ...
      it 'fires side effects via SideFxProjectService' do
        expect { job.run(admin_publication.id, user.id) }
    ...
    context 'archived to published' do
      before do
        admin_publication.update_columns(
          ...
          scheduled_at: 1.hour.ago, first_published_at: 1.day.ago
        )
        admin_publication.reload
      end
    ...
    context 'already at target status' do
      before do
        admin_publication.update_columns(publication_status: 'published', scheduled_status: 'published', scheduled_at: 1.hour.ago)
      end
    ...
    context 'missing record' do
      it 'exits gracefully' do
        expect { job.run('nonexistent-id', user.id) }.not_to raise_error
      end
    end
  end
end
