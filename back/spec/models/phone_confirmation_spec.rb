...
RSpec.describe PhoneConfirmation do
  describe '#confirm!' do
    it 'stamps phone_confirmed_at and clears the confirmation code' do
      ...
      user.phone_confirmation.update!(code: '1234', code_sent_at: Time.zone.now)

      expect(user.phone_confirmation.confirm!).to be true

      user.reload
      expect(user.phone_confirmed_at).to be_present
      expect(user.phone_confirmation.code).to be_nil
    end
  ...
  describe '#generate_code' do
    it "returns '1234' when the sms use_test_mode setting is enabled" do
      ...
      user = create(:user)
      expect(user.phone_confirmation.generate_code).to eq('1234')
    end
  end
  ...
  describe '#pending?' do
    it 'is true only when the phone is set and not yet confirmed' do
      user = create(:user)
      expect(user.phone_confirmation.pending?).to be false

      user.update!(phone: '+14155552671')
      expect(user.phone_confirmation.pending?).to be true
