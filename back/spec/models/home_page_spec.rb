...
RSpec.describe HomePage, type: :model do
  describe 'validations' do
    ...
    context 'when banner_signed_out_type is set to \'customized_button\'' do
      subject { described_class.new(banner_signed_out_type: 'customized_button') }

      it { is_expected.to validate_presence_of(:banner_signed_out_url) }
      it { is_expected.to validate_presence_of(:banner_signed_out_text_multiloc) }
    end

    context 'when banner_signed_in_type is set to \'customized_button\'' do
      subject { described_class.new(banner_signed_in_type: 'customized_button') }

      it { is_expected.to validate_presence_of(:banner_signed_in_url) }
      it { is_expected.to validate_presence_of(:banner_signed_in_text_multiloc) }
    end
