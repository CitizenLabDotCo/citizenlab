...
RSpec.describe InputTypeStrategy::SelectImage do
  subject(:input_type_strategy) { described_class.new(custom_field) }

  let(:custom_field) { build(:custom_field_select_image) }
  ...
  its(:supports_option_images?) { is_expected.to be true }
  ...
  its(:supports_pdf_llm_import?) { is_expected.to be false }
