...
RSpec.describe InputTypeStrategy::MultilineTextMultiloc do
  subject(:input_type_strategy) { described_class.new(custom_field) }

  let(:custom_field) { build(:custom_field, input_type: 'multiline_text_multiloc') }
  ...
  its(:supports_text?) { is_expected.to be true }
