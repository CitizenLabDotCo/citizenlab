...
RSpec.describe InputTypeStrategy::SentimentLinearScale do
  subject(:input_type_strategy) { described_class.new(custom_field) }

  let(:custom_field) { build(:custom_field_sentiment_linear_scale) }

  its(:supports_average?) { is_expected.to be true }
  ...
  its(:supports_follow_up?) { is_expected.to be true }
  its(:supports_text?) { is_expected.to be false }
  its(:supports_linear_scale?) { is_expected.to be true }
  its(:supports_linear_scale_labels?) { is_expected.to be true }
