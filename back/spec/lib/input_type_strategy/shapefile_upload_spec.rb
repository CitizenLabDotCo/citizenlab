...
RSpec.describe InputTypeStrategy::ShapefileUpload do
  subject(:input_type_strategy) { described_class.new(custom_field) }

  let(:custom_field) { build(:custom_field, input_type: 'shapefile_upload') }
  ...
  its(:supports_file_upload?) { is_expected.to be true }
