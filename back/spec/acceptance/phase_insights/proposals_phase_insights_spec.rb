...
resource 'Phase insights' do
  ...
  let(:proposals_phase) do
    create(
      ...
      user2 = create(:user)
      user3 = create(:user)
      user4 = create(:user, custom_field_answers: [build(:custom_field_answer, key: 'gender', value: 'female'), build(:custom_field_answer, key: 'birthyear', value: 1980)])
      user5 = create(:user, custom_field_answers: [build(:custom_field_answer, key: 'gender', value: 'male'), build(:custom_field_answer, key: 'birthyear', value: 1990)])
  ...
  get 'web_api/v1/phases/:id/insights' do
    example_request 'returns insights data for proposals phase' do
      ...
      expect(metrics).to eq({
        ...
        proposals: {
          ideas_posted: 2,
          ideas_posted_7_day_percent_change: 100.0, # from 1 by 7-days ago, to 2 now = 100% increase
          reached_threshold: 2,
          reached_threshold_7_day_percent_change: 100.0, # from 1 by 7-days ago, to 2 now = 100% increase
          comments_posted: 1,
          comments_posted_7_day_percent_change: 0.0, # from 1 by 7-days ago, to 0 now = 0% change
          reactions: 1,
