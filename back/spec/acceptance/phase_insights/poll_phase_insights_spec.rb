...
resource 'Phase insights' do
  ...
  let_it_be(:custom_field_gender, reload: true) { create(:custom_field, resource_type: 'User', key: 'gender', input_type: 'select', title_multiloc: { en: 'Gender' }) }
  ...
  get 'web_api/v1/phases/:id/insights' do
    example_request 'returns insights data for poll phase' do
      ...
      expect(metrics).to eq({
        ...
        participants: 2,
        participants_7_day_percent_change: 100.0, # from 1 unique participant 7-days ago, to 2 now = 100% change
        participation_rate_as_percent: 100.0,
        participation_rate_7_day_percent_change: 100.0, # participation_rate_7_days_ago = 0.5, participation_rate_now = 1.0 = (((1.0 - 0.5).to_f / 0.5) * 100.0).round(1)
        poll: {
          responses: 2,
          responses_7_day_percent_change: 100.0 # from 1 by 7-days ago, to 2 now = 100% change
        }
      })
