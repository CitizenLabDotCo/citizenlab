...
describe FormLogicService do
  ...
  let(:target_page1) { create :custom_field_page, :for_custom_form, resource: field.resource }
  let(:fields) { [front_page, field, target_page1] }
  let(:logic) { {} }
  ...
  describe '#valid?' do
    let(:field) { create(:custom_field_linear_scale, :for_custom_form, logic: logic) }
    ...
    context 'returns true when logic has a good structure' do
      ...
      let(:fields) { [front_page, field, target_page1, target_page2, target_page3, target_page4] }
    ...
    context 'returns false when logic has a bad structure' do
      let(:target_page2) { create :custom_field_page, :for_custom_form, resource: field.resource }
      let(:fields) { [front_page, field, target_page1, target_page2] }
    ...
    context 'when logic has target fields that are not pages' do
      ...
      let(:fields) { [front_page, field, target_page1, end_field] }
    ...
    context 'when the source field is a page' do
      ...
      it 'returns false' do
        field.update!(
          logic: { 'rules' => [{ 'if' => 123, 'then' => [{ 'effect' => 'show', 'target_id' => target_page1.id }] }] }
        )
