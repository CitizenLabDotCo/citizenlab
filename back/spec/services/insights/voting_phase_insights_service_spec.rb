  ...
  describe 'phase_participation_method_metrics' do
    ...
    it 'calculates the correct metrics' do
      ...
      expect(metrics).to eq({
        ...
        associated_ideas: 2,
        online_votes: 5,
        online_votes_7_day_percent_change: 50.0, # from 2 7-days ago, to 3 now = 50% increase
        offline_votes: phase.manual_votes_count,
        voters: 1,
        voters_7_day_percent_change: 0.0, # from 1 7-days ago, to 1 now = 0% change
        comments_posted: 2,
        comments_posted_7_day_percent_change: 0.0 # from 1 7-days ago, to 1 now = 0% change
      })
    end
  end
