...
describe Permissions::PhasePermissionsService do
  ...
  context '"reacting_idea" denied_reason_for_phase' do
    context 'when reacting is enabled' do
      ...
      it 'returns nil' do
        expect(service.denied_reason_for_phase('reacting_idea', reaction_mode: 'up')).to be_nil
        expect(service.denied_reason_for_phase('reacting_idea', reaction_mode: 'down')).to be_nil
      end
    end
    ...
    context 'when the like limit was reached for that phase' do
      ...
      it 'returns `reacting_like_limited_max_reached`' do
        ...
        expect(service.denied_reason_for_phase('reacting_idea', reaction_mode: 'up')).to eq 'reacting_like_limited_max_reached'
        expect(service.denied_reason_for_phase('reacting_idea', reaction_mode: 'down')).to be_nil
      end
    end
  end
