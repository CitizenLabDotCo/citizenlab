...
describe Permissions::PhasePermissionsService do
  ...
  # Reasons that apply to every action, so they are only asserted once here.
  describe 'context denied reasons' do
  ...
  describe 'posting_idea denied_reason_for_action' do
    ...
    context 'permitted group requires verification' do
      ...
      context 'for a verified user' do
        let(:user) { create(:user, verified: true, custom_field_answers: [build(:custom_field_answer, key: 'birthyear', value: 2008)]) }
  ...
  describe '#action_descriptors' do
    it 'returns a descriptor with :enabled and :disabled_reason for every supported action' do
      descriptors = service.action_descriptors
