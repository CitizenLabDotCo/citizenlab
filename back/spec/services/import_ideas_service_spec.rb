...
describe ImportIdeasService do
  ...
  def generate_idea_data(n)
    Array.new(n) do |_|
      { title_multiloc: { 'en' => "Idea #{rand(100_000)}" },
        body_multiloc: { 'en' => Faker::Lorem.sentence },
        topic_titles: Array.new(rand(5)) { rand(Topic.count) }.uniq.map { |offset| Topic.offset(offset).first.title_multiloc.values.first },
