...
RSpec.describe Sms::Sender do
  let(:provider) { instance_double(Sms::Providers::Twilio) }
  ...
  before do
    ...
    allow(Sms::Providers::Twilio).to receive(:new).and_return(provider)
  end
  ...
  describe '#send_now' do
    ...
    it 'marks the delivery failed and re-raises when the provider fails' do
      allow(provider).to receive(:send).and_raise(Sms::Error, 'Provider rejected it')
  ...
  describe '#deliver' do
    ...
    it 'marks the delivery failed and re-raises when the provider fails' do
      allow(provider).to receive(:send).and_raise(Sms::Error, 'nope')
